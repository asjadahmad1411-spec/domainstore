...
function requireAdminAuth() {
  if (!getAdminToken() && !location.pathname.endsWith('/admin') && !location.pathname.endsWith('/admin/')) {
    location.href = '/admin/';
  }
}
...
async function apiFetch(url, options = {}) {
  ...
  if (res.status === 401) { clearAdminToken(); location.href = '/admin/'; throw new Error('Unauthorized'); }
  return res.json();
}
...
document.addEventListener('DOMContentLoaded', () => {
  const isLoginPage = location.pathname === '/admin/' || location.pathname.endsWith('/admin') || location.pathname.endsWith('/admin/index.html');
  if (!isLoginPage) {
    requireAdminAuth();
    initSidebarToggle();
